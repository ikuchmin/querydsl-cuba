...
import ru.udya.querydsl.cuba.core.domain.Animal;
import ru.udya.querydsl.cuba.core.domain.Author;
import ru.udya.querydsl.cuba.core.domain.Book;
...
import ru.udya.querydsl.cuba.core.domain.Entity1;
import ru.udya.querydsl.cuba.core.domain.Entity2;
import ru.udya.querydsl.cuba.core.domain.Foo;
import ru.udya.querydsl.cuba.core.domain.JobFunction;
import ru.udya.querydsl.cuba.core.domain.Numeric;
import ru.udya.querydsl.cuba.core.domain.QAnimal;
import ru.udya.querydsl.cuba.core.domain.QAuthor;
import ru.udya.querydsl.cuba.core.domain.QBook;
import ru.udya.querydsl.cuba.core.domain.QCat;
import ru.udya.querydsl.cuba.core.domain.QCompany;
import ru.udya.querydsl.cuba.core.domain.QDomesticCat;
import ru.udya.querydsl.cuba.core.domain.QDoubleProjection;
import ru.udya.querydsl.cuba.core.domain.QEmployee;
...
import ru.udya.querydsl.cuba.core.qcore.group.MockTuple;
import ru.udya.querydsl.cuba.core.qcore.targets.ExcludeIn;
import ru.udya.querydsl.cuba.core.qcore.targets.NoBatooJPA;
import ru.udya.querydsl.cuba.core.qcore.targets.NoEclipseLink;
import ru.udya.querydsl.cuba.core.qcore.targets.NoHibernate;
import ru.udya.querydsl.cuba.core.qcore.targets.NoOpenJPA;
import ru.udya.querydsl.cuba.core.qcore.types.Concatenation;
...
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.DERBY;
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.HSQLDB;
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.MYSQL;
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.ORACLE;
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.POSTGRESQL;
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.SQLSERVER;
import static ru.udya.querydsl.cuba.core.qcore.targets.Target.TERADATA;

/**
 * @author tiwe
 *
 */
public abstract class AbstractCubaTest {
...
//            path.countDistinct()
    ...
    public void constant() {
        ...
        for (int i = 0; i < cats.size(); i++) {
            assertEquals(Integer.valueOf(cats.get(i).getIntId()), tuples.get(i).get(cat.id));
...
//        group by cat0_.name , cat0_.breed
    ...
    public void groupBy_count() {
        List<UUID> ids = query().from(cat).groupBy(cat.id).select(cat.id).fetch();
        long count = query().from(cat).groupBy(cat.id).fetchCount();
        QueryResults<UUID> results = query().from(cat).groupBy(cat.id)
...
//                .select(new QFloatProjection(cat.doubleProperty.sum())).fetchFirst();
//        assertEquals(val, projection.val, 0.001);
    }
